...
AddressController.prototype.newAddress = function(req, res, next) {
    var self = this;
    this.node.getNewAddress(function(err, addr) {
        if(err) {
            return self.common.handleErrors(err, res);
        }
        res.json({
...
AddressController.prototype.multiutxo = function(req, res) {
  var self = this;
  this.node.getAddressUnspentOutputs(req.addrs, false, function(err, utxos) {
    if(err && err.code === -5) {
      return res.jsonp([]);
    } else if(err) {
      return self.common.handleErrors(err, res);
    ...
    for(var i = 0; i < utxos.length; i++) {
      if ( utxos[i].height ) 
        filteredUtxos[utxos[i].txid] = utxos[i];
      else {
        utxos[i].pos = i; 
        staledUtxos[utxos[i].txid] = utxos[i];
      }	
    }	
    console.log("staled utxos : %s\n", JSON.stringify(staledUtxos));
    console.log("filteredUtxos :%s\n", JSON.stringify(filteredUtxos));	
    for(var txid in staledUtxos ) {
      console.log("filter utxo.%s = %s\n", txid,JSON.stringify(filteredUtxos[txid]));
      console.log("splice pos = %d\n", staledUtxos[txid].pos);
      if( typeof filteredUtxos[txid] != 'undefined' && staledUtxos[txid].pos >= 0) {
          console.log("Doing spliced pos = %d\n", staledUtxos[txid].pos);
          delete utxos[staledUtxos[txid].pos];
      } 
    }	
    var utxos = utxos.filter(function (el) {
      return el != null;
    });
